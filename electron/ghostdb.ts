...
class GhostedIndex {
  ...
  private async scanDir(dir: string, depth = 0): Promise<void> {
    ...
    for (const e of entries) {
      ...
      if (e.isDirectory()) {
        await this.scanDir(full, depth + 1)
        continue
      }
  ...
  private parseFile(filePath: string): void {
    try {
      ...
      if (MD_EXTS.has(ext)) {
        try {
          const p = matter(raw)
          fm = p.data as Record<string, unknown>
          body = p.content
        } catch {}
      }
      ...
      if (MD_EXTS.has(ext)) {
        for (const m of body.matchAll(WL_RE)) wikilinks.push(m[1].trim())
      }
  ...
  private watch(root: string): void {
    this.watcher = chokidar.watch(root, {
      ignored: (p: string) =>
        p.split(path.sep).some((s) => IGNORED.has(s) || (s.startsWith('.') && s !== '.claude')),
      persistent: true,
      ignoreInitial: true,
  ...
  query(q: GhostedQuery): GhostedQueryResult {
      ...
      for (const c of q.where) {
        r = r.filter((f) => {
          ...
          switch (c.op) {
            ...
            case 'contains':
              return typeof v === 'string' && v.toLowerCase().includes((c.value as string).toLowerCase())
            case 'gt':
    ...
    r.sort((a, b) => {
      const av =
        ob === 'name' ? a.name : ob === 'mtime' ? a.mtime : ob === 'size' ? a.size : a.frontmatter[ob]
      const bv =
        ob === 'name' ? b.name : ob === 'mtime' ? b.mtime : ob === 'size' ? b.size : b.frontmatter[ob]
