...
import { type OpenFile, useStore } from '@/store'
    ...
    <>
      ...
      >
        <span className="filetree-content">
          ...
          {jsonIcon(value)}
          <span className="filetree-name" style={{ color: 'var(--text-primary)' }}>
            {label}
          </span>
          {!isExpandable && (
      ...
      {open && entries.map(([key, val]) => <JsonNode key={key} label={key} value={val} depth={depth + 1} />)}
    </>
  )
}
...
export default function EditorPane({ filePath }: { leafId?: string; filePath?: string }) {
  ...
  if (!activeFile) {
    return (
      ...
      >
        ...
        <span
          style={{ fontSize: 24, fontFamily: 'var(--font-mono)', fontWeight: 700, color: 'var(--bg-base)' }}
        >
          open a file to begin haunting
        </span>
      </div>
    )
  }
        ...
        <div style={{ flex: 1, overflow: 'hidden' }}>
          ...
          >
            <MonacoEditor
              ...
              onChange={(v) => {
                if (!filePath || v === undefined) return
