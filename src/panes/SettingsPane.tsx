...
function Toggle({ value, onChange }: { value: boolean; onChange: (v: boolean) => void }) {
  return (
    <button type="button" onClick={() => onChange(!value)} className={`settings-toggle ${value ? 'on' : ''}`}>
      ...
      <div className="settings-scroll">
        ...
        <SettingRow label="Word Wrap">
          <SelectInput
            ...
            onChange={(v) => up('editorWordWrap', v as any)}
            options={[
              { value: 'on', label: 'On' },
              { value: 'off', label: 'Off' },
        ...
        <SettingRow label="Line Numbers">
          <SelectInput
            ...
            onChange={(v) => up('editorLineNumbers', v as any)}
            options={[
              { value: 'on', label: 'On' },
              { value: 'off', label: 'Off' },
        ...
        <SettingRow label="Font Size">
          <NumberInput
            ...
            onChange={(v) => up('terminalFontSize', v)}
            min={8}
            max={32}
          />
        </SettingRow>
        ...
        <SettingRow label="Line Height">
          <NumberInput
            ...
            onChange={(v) => up('terminalLineHeight', v)}
            min={1}
            max={2.5}
            step={0.05}
          />
        ...
        <SettingRow label="Cursor Style">
          <SelectInput
            ...
            onChange={(v) => up('terminalCursorStyle', v as any)}
            options={[
        ...
        <SettingRow label="Scrollback" description="Lines of history">
          <NumberInput
            ...
            onChange={(v) => up('terminalScrollback', v)}
            min={100}
            max={100000}
            step={100}
          />
        ...
        <SettingRow label="Auto Refresh" description="Seconds between status checks">
          <NumberInput
            value={s.gitAutoRefreshInterval}
            onChange={(v) => up('gitAutoRefreshInterval', v)}
            min={1}
            max={60}
          />
        </SettingRow>
        ...
        <SettingRow label="Reset Layout" description="Clear saved layout and reload">
          <button
            type="button"
            onClick={() => {
              localStorage.removeItem('ghosted:layout')
              location.reload()
            }}
            style={{
              padding: '4px 10px',
