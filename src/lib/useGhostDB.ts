...
import type { DBStats, GhostedFile, GhostedQuery, GhostedQueryResult } from '@/types/electron'
...
export function useGhostDB() {
  ...
  const query = useCallback((q: GhostedQuery): Promise<GhostedQueryResult> => {
    return window.electron.db.query(q)
  }, [])

  const get = useCallback((filePath: string): Promise<GhostedFile | null> => {
